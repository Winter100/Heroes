export const prefix_enchant_options = [
  {
    rank: "4",
    stat_name: "복수의",
    stat_value: [
      { stat_name: "공격력", stat_value: "550" },
      { stat_name: "마법공격력", stat_value: "550" },
      { stat_name: "공격속도", stat_value: "8" },
      { stat_name: "크리티컬", stat_value: "6" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "5",
    stat_name: "잔혹한",
    stat_value: [
      { stat_name: "공격력", stat_value: "430" },
      { stat_name: "마법공격력", stat_value: "430" },
      { stat_name: "공격속도", stat_value: "8" },
      { stat_name: "크리티컬", stat_value: "4" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "6",
    stat_name: "차분한",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "공격속도", stat_value: "4" },
      { stat_name: "일정 확률(2)로 SP 125 회복", stat_value: "" },
      { stat_name: "밸런스", stat_value: "5" },
    ],
  },
  {
    rank: "6",
    stat_name: "혼돈의",
    stat_value: [
      { stat_name: "공격력", stat_value: "340" },
      { stat_name: "마법공격력", stat_value: "340" },
      { stat_name: "공격속도", stat_value: "4" },
      { stat_name: "크리티컬", stat_value: "9" },
      { stat_name: "크리티컬 저항", stat_value: "-4" },
    ],
  },
  {
    rank: "7",
    stat_name: "불의의",
    stat_value: [
      { stat_name: "공격력", stat_value: "220" },
      { stat_name: "마법공격력", stat_value: "220" },
      { stat_name: "공격속도", stat_value: "4" },
      { stat_name: "크리티컬", stat_value: "8" },
      { stat_name: "크리티컬 저항", stat_value: "-5" },
    ],
  },
  {
    rank: "7",
    stat_name: "정의로운",
    stat_value: [
      { stat_name: "공격력", stat_value: "430" },
      { stat_name: "마법공격력", stat_value: "430" },
      { stat_name: "공격속도", stat_value: "8" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "8",
    stat_name: "확고한",
    stat_value: [
      { stat_name: "공격속도", stat_value: "4" },
      { stat_name: "밸런스", stat_value: "5" },
    ],
  },
  {
    rank: "8",
    stat_name: "표범의",
    stat_value: [
      { stat_name: "공격력", stat_value: "-240" },
      { stat_name: "마법공격력", stat_value: "-240" },
      { stat_name: "공격속도", stat_value: "8" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "8",
    stat_name: "풍요로운",
    stat_value: [
      { stat_name: "공격력", stat_value: "120" },
      { stat_name: "마법공격력", stat_value: "120" },
      { stat_name: "공격속도", stat_value: "5" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "밸런스", stat_value: "1" },
    ],
  },
  {
    rank: "9",
    stat_name: "빛과 같은",
    stat_value: [
      { stat_name: "공격력", stat_value: "180" },
      { stat_name: "마법공격력", stat_value: "180" },
      { stat_name: "공격속도", stat_value: "3" },
    ],
  },
  {
    rank: "9",
    stat_name: "결정적인",
    stat_value: [
      { stat_name: "공격력", stat_value: "120" },
      { stat_name: "마법공격력", stat_value: "120" },
      { stat_name: "크리티컬", stat_value: "3" },
      { stat_name: "밸런스", stat_value: "1" },
    ],
  },
  {
    rank: "A",
    stat_name: "용감한",
    stat_value: [
      { stat_name: "공격력", stat_value: "110" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "A",
    stat_name: "고풍스러운",
    stat_value: [
      { stat_name: "공격력", stat_value: "320" },
      { stat_name: "마법공격력", stat_value: "320" },
    ],
  },
  {
    rank: "A",
    stat_name: "수호의",
    stat_value: [
      { stat_name: "방어력", stat_value: "112" },
      { stat_name: "생명력", stat_value: "50" },
    ],
  },
  {
    rank: "9",
    stat_name: "노력의",
    stat_value: [
      { stat_name: "방어력", stat_value: "35" },
      { stat_name: "생명력", stat_value: "50" },
      { stat_name: "공격속도", stat_value: "1" },
    ],
  },
  {
    rank: "8",
    stat_name: "균형 잡힌",
    stat_value: [
      { stat_name: "공격속도", stat_value: "2" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "-1" },
      { stat_name: "밸런스", stat_value: "1" },
    ],
  },
  {
    rank: "8",
    stat_name: "꾸준한",
    stat_value: [{ stat_name: "스태미나", stat_value: "15" }],
  },
  {
    rank: "7",
    stat_name: "시간의",
    stat_value: [
      { stat_name: "방어력", stat_value: "160" },
      { stat_name: "생명력", stat_value: "50" },
      { stat_name: "스태미나", stat_value: "5" },
      { stat_name: "크리티컬", stat_value: "4" },
      { stat_name: "크리티컬 저항", stat_value: "6" },
      { stat_name: "밸런스", stat_value: "-3" },
    ],
  },
  {
    rank: "6",
    stat_name: "무한의",
    stat_value: [
      { stat_name: "방어력", stat_value: "160" },
      { stat_name: "생명력", stat_value: "50" },
      { stat_name: "스태미나", stat_value: "5" },
      { stat_name: "크리티컬", stat_value: "5" },
      { stat_name: "크리티컬 저항", stat_value: "8" },
      { stat_name: "밸런스", stat_value: "-3" },
    ],
  },
  {
    rank: "5",
    stat_name: "고결한",
    stat_value: [
      { stat_name: "방어력", stat_value: "160" },
      { stat_name: "생명력", stat_value: "100" },
      { stat_name: "스태미나", stat_value: "8" },
      { stat_name: "크리티컬", stat_value: "6" },
      { stat_name: "크리티컬 저항", stat_value: "10" },
      { stat_name: "밸런스", stat_value: "-3" },
    ],
  },
  {
    rank: "4",
    stat_name: "예언의",
    stat_value: [
      { stat_name: "방어력", stat_value: "160" },
      { stat_name: "생명력", stat_value: "100" },
      { stat_name: "스태미나", stat_value: "8" },
      { stat_name: "크리티컬", stat_value: "7" },
      { stat_name: "크리티컬 저항", stat_value: "12" },
      { stat_name: "밸런스", stat_value: "-3" },
    ],
  },
  {
    rank: "7",
    stat_name: "기억하는",
    stat_value: [
      { stat_name: "공격력", stat_value: "245" },
      { stat_name: "마법공격력", stat_value: "245" },
      { stat_name: "방어력", stat_value: "100" },
      { stat_name: "공격속도", stat_value: "2" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "크리티컬 저항", stat_value: "2" },
      { stat_name: "밸런스", stat_value: "1" },
    ],
  },
  {
    rank: "6",
    stat_name: "냉정의",
    stat_value: [
      { stat_name: "공격력", stat_value: "245" },
      { stat_name: "마법공격력", stat_value: "245" },
      { stat_name: "방어력", stat_value: "200" },
      { stat_name: "공격속도", stat_value: "2" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "크리티컬 저항", stat_value: "4" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "5",
    stat_name: "냉철의",
    stat_value: [
      { stat_name: "공격력", stat_value: "245" },
      { stat_name: "마법공격력", stat_value: "245" },
      { stat_name: "방어력", stat_value: "200" },
      { stat_name: "공격속도", stat_value: "2" },
      { stat_name: "크리티컬", stat_value: "3" },
      { stat_name: "크리티컬 저항", stat_value: "6" },
      { stat_name: "밸런스", stat_value: "3" },
    ],
  },
  {
    rank: "4",
    stat_name: "잊혀진",
    stat_value: [
      { stat_name: "공격력", stat_value: "245" },
      { stat_name: "마법공격력", stat_value: "245" },
      { stat_name: "방어력", stat_value: "200" },
      { stat_name: "공격속도", stat_value: "2" },
      { stat_name: "크리티컬", stat_value: "3" },
      { stat_name: "크리티컬 저항", stat_value: "8" },
      { stat_name: "밸런스", stat_value: "4" },
    ],
  },
  {
    rank: "7",
    stat_name: "되뇌는",
    stat_value: [
      { stat_name: "공격력", stat_value: "145" },
      { stat_name: "마법공격력", stat_value: "145" },
      { stat_name: "방어력", stat_value: "150" },
      { stat_name: "공격속도", stat_value: "2" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "-1" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "6",
    stat_name: "흐느끼는",
    stat_value: [
      { stat_name: "공격력", stat_value: "145" },
      { stat_name: "마법공격력", stat_value: "145" },
      { stat_name: "방어력", stat_value: "300" },
      { stat_name: "공격속도", stat_value: "2" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "크리티컬 저항", stat_value: "-1" },
      { stat_name: "밸런스", stat_value: "3" },
    ],
  },
  {
    rank: "5",
    stat_name: "비통한",
    stat_value: [
      { stat_name: "공격력", stat_value: "145" },
      { stat_name: "마법공격력", stat_value: "145" },
      { stat_name: "방어력", stat_value: "300" },
      { stat_name: "공격속도", stat_value: "2" },
      { stat_name: "크리티컬", stat_value: "3" },
      { stat_name: "크리티컬 저항", stat_value: "-1" },
      { stat_name: "밸런스", stat_value: "4" },
    ],
  },
  {
    rank: "4",
    stat_name: "우아한",
    stat_value: [
      { stat_name: "공격력", stat_value: "145" },
      { stat_name: "마법공격력", stat_value: "145" },
      { stat_name: "방어력", stat_value: "300" },
      { stat_name: "공격속도", stat_value: "2" },
      { stat_name: "크리티컬", stat_value: "4" },
      { stat_name: "크리티컬 저항", stat_value: "-1" },
      { stat_name: "밸런스", stat_value: "5" },
    ],
  },
  {
    rank: "A",
    stat_name: "강인한",
    stat_value: [
      { stat_name: "방어력", stat_value: "35" },
      { stat_name: "스태미나", stat_value: "2" },
      { stat_name: "힘", stat_value: "9" },
      { stat_name: "지능", stat_value: "9" },
    ],
  },
  {
    rank: "9",
    stat_name: "호기심 많은",
    stat_value: [
      { stat_name: "방어력", stat_value: "130" },
      { stat_name: "힘", stat_value: "6" },
      { stat_name: "지능", stat_value: "6" },
    ],
  },
  {
    rank: "8",
    stat_name: "의미있는",
    stat_value: [
      { stat_name: "공격력", stat_value: "15" },
      { stat_name: "마법공격력", stat_value: "15" },
      { stat_name: "공격속도", stat_value: "1" },
    ],
  },
  {
    rank: "8",
    stat_name: "신속한",
    stat_value: [
      { stat_name: "공격속도", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "-8" },
    ],
  },
  {
    rank: "6",
    stat_name: "뜻깊은",
    stat_value: [
      { stat_name: "공격력", stat_value: "200" },
      { stat_name: "마법공격력", stat_value: "200" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "3" },
    ],
  },
  {
    rank: "5",
    stat_name: "뒤틀린",
    stat_value: [
      { stat_name: "공격력", stat_value: "450" },
      { stat_name: "마법공격력", stat_value: "450" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
    ],
  },
  {
    rank: "8",
    stat_name: "반짝이는",
    stat_value: [{ stat_name: "생명력", stat_value: "-280" }],
  },
  {
    rank: "7",
    stat_name: "죽은자의",
    stat_value: [
      { stat_name: "생명력", stat_value: "-450" },
      { stat_name: "힘", stat_value: "-40" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "5" },
    ],
  },
  {
    rank: "5",
    stat_name: "추적자의",
    stat_value: [
      { stat_name: "생명력", stat_value: "-450" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "7" },
    ],
  },
  {
    rank: "4",
    stat_name: "대적자의",
    stat_value: [
      { stat_name: "생명력", stat_value: "-450" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "9" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "8",
    stat_name: "조그만",
    stat_value: [
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "-3" },
    ],
  },
  {
    rank: "7",
    stat_name: "은은한",
    stat_value: [
      { stat_name: "스태미나", stat_value: "5" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "크리티컬 저항", stat_value: "-3" },
    ],
  },
  {
    rank: "5",
    stat_name: "강렬한",
    stat_value: [
      { stat_name: "스태미나", stat_value: "5" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬", stat_value: "3" },
      { stat_name: "크리티컬 저항", stat_value: "-3" },
      { stat_name: "밸런스", stat_value: "1" },
    ],
  },
  {
    rank: "4",
    stat_name: "고대의",
    stat_value: [{ stat_name: "스태미나", stat_value: "5" }],
  },
  {
    rank: "5",
    stat_name: "시공의",
    stat_value: [
      { stat_name: "공격력", stat_value: "450" },
      { stat_name: "마법 공격력", stat_value: "450" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
      { stat_name: "추가피해", stat_value: "100" },
      { stat_name: "크리티컬", stat_value: "4" },
      { stat_name: "크리티컬 저항", stat_value: "-3" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "5",
    stat_name: "신중한",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "공격속도", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "7" },
      { stat_name: "SP 회복 확률", stat_value: "125" },
      { stat_name: "고유 그룹", stat_value: "자원 회복" },
      { stat_name: "공격력 제한 해제", stat_value: "50" },
    ],
  },
  {
    rank: "6",
    stat_name: "차분한",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "공격속도", stat_value: "4" },
      { stat_name: "밸런스", stat_value: "5" },
      { stat_name: "SP 회복 확률", stat_value: "125" },
      { stat_name: "고유 그룹", stat_value: "자원 회복" },
    ],
  },
  {
    rank: "7",
    stat_name: "조용한",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "밸런스", stat_value: "4" },
      { stat_name: "공격속도", stat_value: "3" },
      { stat_name: "SP 회복 확률", stat_value: "125" },
      { stat_name: "고유 그룹", stat_value: "자원 회복" },
    ],
  },
  {
    rank: "5",
    stat_name: "집중의",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "공격속도", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "7" },
      { stat_name: "마나 회복", stat_value: "2" },
      { stat_name: "고유 그룹", stat_value: "자원 회복" },
      { stat_name: "공격력 제한 해제", stat_value: "50" },
    ],
  },
  {
    rank: "6",
    stat_name: "평온한",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "밸런스", stat_value: "5" },
      { stat_name: "공격속도", stat_value: "4" },
      { stat_name: "마나 회복", stat_value: "2" },
      { stat_name: "고유 그룹", stat_value: "자원 회복" },
    ],
  },
  {
    rank: "7",
    stat_name: "명상의",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "밸런스", stat_value: "4" },
      { stat_name: "공격속도", stat_value: "3" },
      { stat_name: "마나 회복", stat_value: "2" },
      { stat_name: "고유 그룹", stat_value: "자원 회복" },
    ],
  },
  {
    rank: "5",
    stat_name: "단호한",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "공격속도", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "7" },
      { stat_name: "SP 회복 확률", stat_value: "125" },
      { stat_name: "고유 그룹", stat_value: "자원 회복" },
    ],
  },
  {
    rank: "6",
    stat_name: "결연한",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "밸런스", stat_value: "5" },
      { stat_name: "공격속도", stat_value: "4" },
      { stat_name: "SP 회복 확률", stat_value: "125" },
      { stat_name: "고유 그룹", stat_value: "자원 회복" },
    ],
  },
  {
    rank: "7",
    stat_name: "결심의",
    stat_value: [
      { stat_name: "공격력", stat_value: "-280" },
      { stat_name: "마법공격력", stat_value: "-280" },
      { stat_name: "밸런스", stat_value: "4" },
      { stat_name: "공격속도", stat_value: "3" },
      { stat_name: "SP 회복 확률", stat_value: "125" },
      { stat_name: "고유 그룹", stat_value: "자원 회복" },
    ],
  },
  {
    rank: "A",
    stat_name: "맷집 좋은",
    stat_value: [{ stat_name: "방어력", stat_value: "190" }],
  },
  {
    rank: "A",
    stat_name: "현자의",
    stat_value: [
      { stat_name: "민첩", stat_value: "-6" },
      { stat_name: "힘", stat_value: "15" },
      { stat_name: "지능", stat_value: "21" },
    ],
  },
  {
    rank: "9",
    stat_name: "짜릿한",
    stat_value: [
      { stat_name: "방어력", stat_value: "145" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "-3" },
    ],
  },
  {
    rank: "7",
    stat_name: "산뜻한",
    stat_value: [
      { stat_name: "공격력", stat_value: "55" },
      { stat_name: "마법 공격력", stat_value: "55" },
      { stat_name: "공격속도", stat_value: "3" },
    ],
  },
  {
    rank: "7",
    stat_name: "봉인된",
    stat_value: [
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "방어력", stat_value: "500" },
    ],
  },
  {
    rank: "5",
    stat_name: "격렬한",
    stat_value: [
      { stat_name: "방어력", stat_value: "800" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "3" },
    ],
  },
  {
    rank: "4",
    stat_name: "승자의",
    stat_value: [
      { stat_name: "방어력", stat_value: "800" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
    ],
  },
];

export const suffix_enchant_options = [
  {
    rank: "A",
    stat_name: "약화",
    stat_value: [
      { stat_name: "공격력", stat_value: "35" },
      { stat_name: "마법공격력", stat_value: "35" },
      { stat_name: "공격속도", stat_value: "1" },
    ],
  },
  {
    rank: "A",
    stat_name: "보호막",
    stat_value: [
      { stat_name: "공격력", stat_value: "150" },
      { stat_name: "마법공격력", stat_value: "150" },
    ],
  },
  {
    rank: "9",
    stat_name: "야망",
    stat_value: [
      { stat_name: "공격력", stat_value: "752" },
      { stat_name: "마법공격력", stat_value: "752" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "밸런스", stat_value: "-2" },
    ],
  },
  {
    rank: "9",
    stat_name: "도전",
    stat_value: [
      { stat_name: "공격력", stat_value: "180" },
      { stat_name: "마법공격력", stat_value: "180" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "밸런스", stat_value: "1" },
    ],
  },
  {
    rank: "8",
    stat_name: "꽃잎",
    stat_value: [
      { stat_name: "공격력", stat_value: "150" },
      { stat_name: "마법공격력", stat_value: "150" },
      { stat_name: "공격속도", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "3" },
    ],
  },
  {
    rank: "8",
    stat_name: "천벌",
    stat_value: [
      { stat_name: "공격력", stat_value: "677" },
      { stat_name: "마법 공격력", stat_value: "677" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬", stat_value: "2" },
    ],
  },
  {
    rank: "8",
    stat_name: "용맹",
    stat_value: [
      { stat_name: "공격력", stat_value: "130" },
      { stat_name: "마법공격력", stat_value: "130" },
      { stat_name: "공격속도", stat_value: "4" },
      { stat_name: "밸런스", stat_value: "5" },
    ],
  },
  {
    rank: "7",
    stat_name: "단죄",
    stat_value: [
      { stat_name: "공격력", stat_value: "680" },
      { stat_name: "마법공격력", stat_value: "680" },
      { stat_name: "크리티컬", stat_value: "6" },
    ],
  },
  {
    rank: "7",
    stat_name: "심판",
    stat_value: [
      { stat_name: "공격력", stat_value: "870" },
      { stat_name: "마법 공격력", stat_value: "870" },
      { stat_name: "공격속도", stat_value: "3" },
      { stat_name: "크리티컬", stat_value: "4" },
      { stat_name: "크리티컬 저항", stat_value: "-1" },
      { stat_name: "밸런스", stat_value: "-1" },
    ],
  },
  {
    rank: "6",
    stat_name: "신념",
    stat_value: [
      { stat_name: "공격력", stat_value: "970" },
      { stat_name: "마법 공격력", stat_value: "970" },
      { stat_name: "공격속도", stat_value: "3" },
      { stat_name: "크리티컬", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "-1" },
    ],
  },
  {
    rank: "5",
    stat_name: "원한",
    stat_value: [
      { stat_name: "공격력", stat_value: "1000" },
      { stat_name: "마법 공격력", stat_value: "1000" },
      { stat_name: "공격속도", stat_value: "3" },
      { stat_name: "크리티컬", stat_value: "7" },
    ],
  },
  {
    rank: "4",
    stat_name: "일격",
    stat_value: [
      { stat_name: "공격력", stat_value: "1030" },
      { stat_name: "마법 공격력", stat_value: "1030" },
      { stat_name: "공격속도", stat_value: "3" },
      { stat_name: "크리티컬", stat_value: "10" },
      { stat_name: "밸런스", stat_value: "-1" },
    ],
  },
  {
    rank: "A",
    stat_name: "치명",
    stat_value: [
      { stat_name: "크리티컬", stat_value: "3" },
      { stat_name: "밸런스", stat_value: "-1" },
    ],
  },
  {
    rank: "A",
    stat_name: "용기",
    stat_value: [
      { stat_name: "공격력", stat_value: "70" },
      { stat_name: "마법공격력", stat_value: "70" },
      { stat_name: "방어력", stat_value: "50" },
      { stat_name: "밸런스", stat_value: "1" },
    ],
  },
  {
    rank: "9",
    stat_name: "아르마딜로",
    stat_value: [
      { stat_name: "방어력", stat_value: "336" },
      { stat_name: "스태미나", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "50 (60초간, 일정 확률)" },
    ],
  },
  {
    rank: "9",
    stat_name: "자스민",
    stat_value: [
      { stat_name: "공격력", stat_value: "140" },
      { stat_name: "마법공격력", stat_value: "140" },
      { stat_name: "방어력", stat_value: "-28" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "8",
    stat_name: "열의",
    stat_value: [
      { stat_name: "공격력", stat_value: "282" },
      { stat_name: "마법공격력", stat_value: "282" },
      { stat_name: "방어력", stat_value: "-370" },
      { stat_name: "밸런스", stat_value: "5" },
    ],
  },
  {
    rank: "8",
    stat_name: "대항",
    stat_value: [
      { stat_name: "방어력", stat_value: "340" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
      {
        stat_name: "생명력 회복",
        stat_value: "50 (10초간, 1초마다, 일정 확률)",
      },
    ],
  },
  {
    rank: "8",
    stat_name: "낙엽",
    stat_value: [
      { stat_name: "방어력", stat_value: "320" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "2" },
      { stat_name: "밸런스", stat_value: "1" },
    ],
  },
  {
    rank: "8",
    stat_name: "보호",
    stat_value: [
      { stat_name: "크리티컬", stat_value: "5" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "-1" },
    ],
  },
  {
    rank: "7",
    stat_name: "낙인",
    stat_value: [
      { stat_name: "방어력", stat_value: "70" },
      { stat_name: "스태미나", stat_value: "5" },
      { stat_name: "크리티컬", stat_value: "6" },
      { stat_name: "크리티컬 저항", stat_value: "7" },
      { stat_name: "밸런스", stat_value: "-1" },
    ],
  },
  {
    rank: "6",
    stat_name: "결계",
    stat_value: [
      { stat_name: "방어력", stat_value: "120" },
      { stat_name: "스태미나", stat_value: "5" },
      { stat_name: "크리티컬", stat_value: "7" },
      { stat_name: "크리티컬 저항", stat_value: "8" },
      { stat_name: "밸런스", stat_value: "-1" },
    ],
  },
  {
    rank: "5",
    stat_name: "한탄",
    stat_value: [
      { stat_name: "방어력", stat_value: "120" },
      { stat_name: "스태미나", stat_value: "5" },
      { stat_name: "크리티컬", stat_value: "9" },
      { stat_name: "크리티컬 저항", stat_value: "10" },
      { stat_name: "밸런스", stat_value: "-1" },
    ],
  },
  {
    rank: "7",
    stat_name: "원정",
    stat_value: [
      { stat_name: "공격력", stat_value: "285" },
      { stat_name: "마법공격력", stat_value: "285" },
      { stat_name: "방어력", stat_value: "280" },
      { stat_name: "크리티컬", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "4" },
    ],
  },
  {
    rank: "6",
    stat_name: "포획",
    stat_value: [
      { stat_name: "공격력", stat_value: "285" },
      { stat_name: "마법공격력", stat_value: "285" },
      { stat_name: "방어력", stat_value: "280" },
      { stat_name: "크리티컬", stat_value: "3" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
    ],
  },
  {
    rank: "5",
    stat_name: "진격",
    stat_value: [
      { stat_name: "공격력", stat_value: "285" },
      { stat_name: "마법공격력", stat_value: "285" },
      { stat_name: "방어력", stat_value: "280" },
      { stat_name: "크리티컬", stat_value: "5" },
      { stat_name: "크리티컬 저항", stat_value: "7" },
    ],
  },
  {
    rank: "7",
    stat_name: "메아리",
    stat_value: [
      { stat_name: "공격력", stat_value: "365" },
      { stat_name: "마법공격력", stat_value: "365" },
      { stat_name: "방어력", stat_value: "-260" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "2" },
      { stat_name: "밸런스", stat_value: "4" },
    ],
  },
  {
    rank: "6",
    stat_name: "영혼",
    stat_value: [
      { stat_name: "공격력", stat_value: "465" },
      { stat_name: "마법공격력", stat_value: "465" },
      { stat_name: "방어력", stat_value: "-300" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "3" },
      { stat_name: "밸런스", stat_value: "5" },
    ],
  },
  {
    rank: "5",
    stat_name: "광분",
    stat_value: [
      { stat_name: "공격력", stat_value: "465" },
      { stat_name: "마법공격력", stat_value: "465" },
      { stat_name: "방어력", stat_value: "-300" },
      { stat_name: "공격속도", stat_value: "1" },
      { stat_name: "크리티컬 저항", stat_value: "5" },
      { stat_name: "밸런스", stat_value: "7" },
    ],
  },
  {
    rank: "4",
    stat_name: "투혼",
    stat_value: [
      { stat_name: "공격력", stat_value: "285" },
      { stat_name: "마법공격력", stat_value: "285" },
      { stat_name: "방어력", stat_value: "280" },
      { stat_name: "크리티컬", stat_value: "8" },
      { stat_name: "크리티컬 저항", stat_value: "10" },
    ],
  },
  {
    rank: "A",
    stat_name: "고통",
    stat_value: [
      { stat_name: "공격력", stat_value: "10" },
      { stat_name: "마법공격력", stat_value: "10" },
      { stat_name: "방어력", stat_value: "20" },
      { stat_name: "생명력", stat_value: "-5" },
    ],
  },
  {
    rank: "9",
    stat_name: "활력",
    stat_value: [
      { stat_name: "방어력", stat_value: "130" },
      { stat_name: "생명력", stat_value: "12" },
    ],
  },
  {
    rank: "8",
    stat_name: "열망",
    stat_value: [
      { stat_name: "방어력", stat_value: "200" },
      { stat_name: "스태미나", stat_value: "2" },
    ],
  },
  {
    rank: "8",
    stat_name: "마음",
    stat_value: [
      { stat_name: "공격력", stat_value: "30" },
      { stat_name: "마법 공격력", stat_value: "30" },
      { stat_name: "스태미나", stat_value: "소량 회복 2단계 (지속)" },
    ],
  },
  {
    rank: "6",
    stat_name: "소망",
    stat_value: [
      { stat_name: "방어력", stat_value: "300" },
      { stat_name: "스태미나", stat_value: "2" },
      { stat_name: "크리티컬 저항", stat_value: "1" },
    ],
  },
  {
    rank: "5",
    stat_name: "격노",
    stat_value: [
      { stat_name: "방어력", stat_value: "400" },
      { stat_name: "스태미나", stat_value: "2" },
      { stat_name: "크리티컬 저항", stat_value: "2" },
    ],
  },
  {
    rank: "5",
    stat_name: "신비",
    equipment: ["반지", "벨트"],
    stat_value: [
      { stat_name: "공격력", stat_value: "200" },
      { stat_name: "마법 공격력", stat_value: "200" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "밸런스", stat_value: "2" },
    ],
  },
  {
    rank: "5",
    stat_name: "돌진",
    stat_value: [
      { stat_name: "발동시 공격 속도", stat_value: "증가" },
      { stat_name: "고유 그룹", stat_value: "공격 속도 증가" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "공격력 제한 해제", stat_value: "100 증가" },
    ],
  },
  {
    rank: "6",
    stat_name: "질주",
    stat_value: [
      { stat_name: "발동시 공격 속도", stat_value: "증가" },
      { stat_name: "고유 그룹", stat_value: "공격 속도 증가" },
    ],
  },
  {
    rank: "7",
    stat_name: "산책",
    stat_value: [
      { stat_name: "크리티컬", stat_value: "-1" },
      { stat_name: "크리티컬 저항", stat_value: "-1" },
      { stat_name: "발동시 공격 속도", stat_value: "증가" },
      { stat_name: "고유 그룹", stat_value: "공격 속도 증가" },
    ],
  },
  {
    rank: "5",
    stat_name: "격돌",
    stat_value: [
      {
        stat_name: "발동시 공격속도",
        stat_value: "증가 (30초마다 발동 가능)",
      },
      { stat_name: "고유 그룹", stat_value: "공격 속도 증가" },
      { stat_name: "크리티컬", stat_value: "2" },
      { stat_name: "공격력 제한 해제", stat_value: "100 증가" },
    ],
  },
  {
    rank: "6",
    stat_name: "폭렬",
    stat_value: [
      {
        stat_name: "발동시 공격속도",
        stat_value: "증가 (30초마다 발동 가능)",
      },
      { stat_name: "고유 그룹", stat_value: "공격 속도 증가" },
    ],
  },
  {
    rank: "7",
    stat_name: "열렬",
    stat_value: [
      { stat_name: "크리티컬", stat_value: "-1" },
      { stat_name: "크리티컬 저항", stat_value: "-1" },
      {
        stat_name: "발동시 공격속도",
        stat_value: "증가 (30초마다 발동 가능)",
      },
      { stat_name: "고유 그룹", stat_value: "공격속도 증가" },
    ],
  },
];

export const prefix_enchant_name_list: { name: string[]; list: string[] }[] = [
  {
    name: ["Right Hand"],
    list: [
      "복수의",
      "잔혹한",
      "차분한",
      "혼돈의",
      "불의의",
      "정의로운",
      "확고한",
      "표범의",
      "풍요로운",
      "빛과 같은",
      "결정적인",
      "용감한",
      "고풍스러운",
    ],
  },
  { name: ["Left Hand"], list: [] },
  {
    name: ["Head", "Lower"],
    list: [
      "잊혀진",
      "냉철의",
      "냉정의",
      "기억하는",
      "균형 잡힌",
      "노력의",
      "수호의",
    ],
  },
  {
    name: ["Upper"],
    list: [
      "예언의",
      "고결한",
      "무한의",
      "시간의",
      "균형 잡힌",
      "꾸준한",
      "노력의",
      "수호의",
    ],
  },
  {
    name: ["Hand", "Leg"],
    list: [
      "우아한",
      "비통한",
      "흐느끼는",
      "되뇌는",
      "균형 잡힌",
      "노력의",
      "수호의",
    ],
  },
  {
    name: ["Right Finger", "Left Finger"],
    list: [
      "대적자의",
      "뒤틀린",
      "추적자의",
      "뜻깊은",
      "죽은자의",
      "반짝이는",
      "신속한",
      "의미있는",
      "호기심 많은",
      "강인한",
    ],
  },
  {
    name: ["Earring", "Charm", "Necklace", "Artifact"],
    list: ["뒤틀린", "뜻깊은", "의미있는", "신속한", "호기심 많은", "강인한"],
  },
  {
    name: ["Belt"],
    list: [
      "고대의",
      "강렬한",
      "뒤틀린",
      "뜻깊은",
      "은은한",
      "의미있는",
      "신속한",
      "조그만",
      "호기심 많은",
      "강인한",
    ],
  },
  {
    name: ["Rhod"],
    list: [
      "신중한",
      "집중의",
      "단호한",
      "결연한",
      "차분한",
      "평온한",
      "결심의",
      "조용한",
      "명상의",
    ],
  },
];

export const suffix_enchant_name_list: { name: string[]; list: string[] }[] = [
  {
    name: ["Right Hand"],
    list: [
      "일격",
      "원한",
      "신념",
      "단죄",
      "심판",
      "천벌",
      "용맹",
      "꽃잎",
      "야망",
      "도전",
      "보호막",
      "약화",
    ],
  },
  { name: ["Left Hand"], list: [] },
  {
    name: ["Head", "Lower"],
    list: [
      "투혼",
      "진격",
      "질주",
      "포획",
      "원정",
      "열의",
      "대항",
      "낙엽",
      "아르마딜로",
      "자스민",
      "용기",
      "치명",
    ],
  },
  {
    name: ["Upper"],
    list: [
      "한탄",
      "결계",
      "질주",
      "낙인",
      "보호막",
      "열의",
      "대항",
      "낙엽",
      "자스민",
      "아르마딜로",
      "치명",
      "용기",
    ],
  },
  {
    name: ["Hand", "Leg"],
    list: [
      "광분",
      "질주",
      "영혼",
      "메아리",
      "대항",
      "열의",
      "낙엽",
      "자스민",
      "아르마딜로",
      "치명",
      "용기",
    ],
  },
  {
    name: ["Right Finger", "Left Finger", "Belt"],
    list: ["신비", "격노", "소망", "마음", "열망", "활력", "고통"],
  },
  {
    name: ["Earring", "Charm", "Necklace", "Artifact"],
    list: ["격노", "소망", "마음", "열망", "활력", "고통"],
  },
  {
    name: ["Rhod"],
    list: ["격돌", "돌진", "폭렬", "질주", "열렬", "산책"],
  },
];
